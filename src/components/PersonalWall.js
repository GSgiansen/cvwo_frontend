...
const PersonalWall = ({posts,delPost,testID}) => {
  ...
  return (
    ...
    <div className='personalwall'>
        ...
        {posts.map((post) => <Indivpost testID={testID} key={post.id} id={post.id} userid={post.title} content={post.body} tag={post.tag} onDelete={delPost}/>)}
    </div>
